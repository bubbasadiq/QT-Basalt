...
class SafetyMonitor:
    ...
    def check_reading(
        ...
        if reading.furnace_temp_c is not None:
            if reading.furnace_temp_c > cfg.safety_max_temp_c + 2.0:  # 2°C tolerance for sensor noise
                breaches.append(
                    f"furnace_temp_c={reading.furnace_temp_c:.1f}°C "
                    f"exceeds max {cfg.safety_max_temp_c}°C"
                )
            elif reading.furnace_temp_c < cfg.safety_min_temp_c - 2.0:  # 2°C tolerance for sensor noise
                breaches.append(
                    f"furnace_temp_c={reading.furnace_temp_c:.1f}°C "
