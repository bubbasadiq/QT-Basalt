...
class ProcessSimulator:
    ...
    def start_run(self, recipe: FiberRecipe) -> ProductionRun:
        ...
        # Pre-heat: start near target so simulation reaches melt temp quickly
        self.furnace.state.temp_c     = recipe.initial_temp_c * 0.95
        self.furnace.state.setpoint_c = recipe.initial_temp_c
    ...
    def step(
        ...
        is_molten = furnace_state.temp_c >= 1100.0
        if is_molten:
        ...
        reading = SensorReading(
            ...
            melt_viscosity_cp  = self._noisy(visc_cp, scale=visc_cp * 0.015) if visc_cp else None,
            melt_flow_rate     = self._noisy(draw_state.mass_flow_g_min, scale=0.5) if draw_state else None,
            fiber_diameter_um  = self._noisy(draw_state.fiber_diameter_um, scale=0.3) if draw_state else 0.0,
            draw_speed_ms      = self._noisy(self._draw_speed_ms, scale=0.05),
            draw_tension_n     = self._noisy(draw_state.draw_tension_n, scale=0.02) if draw_state else None,
